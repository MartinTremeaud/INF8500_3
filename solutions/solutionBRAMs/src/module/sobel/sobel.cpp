...
// Section 1: A completer (1 instruction)
		...
		// Padding of line 0
Line0:	for (unsigned int i = 0; i < img_width_slice; i++)
#pragma HLS unroll
...
L1:		for (unsigned int it = 1; it < nbIterations-1; it++)
		{
...
L2:			for (unsigned int j = 1; j < img_width_slice-1; ++j) {
#pragma HLS pipeline II=1
			    ...
			    w0[0] = w0[1]; w0[1] = w0[2]; w0[2] = p0;
			    w1[0] = w1[1]; w1[1] = w1[2]; w1[2] = p1;
			    w2[0] = w2[1]; w2[1] = w2[2]; w2[2] = p2;
			    ...
			    // Gy
			     ...
			     // seuillage
			     if (edge_val > 200)      out_gray = 255;
			     else if (edge_val < 100) out_gray = 0;
			     else                     out_gray = edge_val;
		...
		// Last Line padding
Last:	for (unsigned int i = 0; i < img_width_slice; i++)
#pragma HLS unroll
			buf_output_pixel[i] = 0;
